...
nCicP = N-K;
fs=10e6; %frequencia de amostragem (10mhz)
...
%-------------------------------------------------------------------------
%esse pega cada valor de C, e substitui toda a coluna pela sua respectiva
%linha de bits
...
% inv=ifft(qam_sum, nFFT);
% inv_plot=stem(inv);
% inv_plot.LineStyle="none";
% inv_plot.Marker = ".";             
% inv_plot.MarkerSize = 1;
% inv_plot.MarkerFaceColor = "auto";
%-------------------------------------------------------------------------
...
%% Imperfeiçoes de microcontroladores
%-------------------------------------%
%            IMPERFEIÇOES             %
%-------------------------------------%

%Conversor ADC/DAC
%Assumindo o uso de um ESP32,
%quantizaçao uniforme   
% Parâmetros do conversor do ESP32(seguindo o datasheet do produto)
bits_adc = 12;
bits_dac = 8;
niveis = 2^bits_adc;

max_tx=max(abs(x_Cic));
Delta_tx= 2*max_tx/(2^bits_dac);
...
y_Cic= awgn(x_Cic_Quantizado,SNR_dB,'measured');
%-------------------------------------
...
max_rx = max(abs(y_Cic));
Delta_rx = 2 * max_rx/(2^bits_adc);


rx_re = round(real(y_Cic)/Delta_rx)*Delta_rx;
rx_im = round(imag(y_Cic)/Delta_rx)*Delta_rx;
y_Cic_Quantizado = rx_re + (1i*rx_im);


y_tempo=y_Cic_Quantizado(nCicP+1:end);
